...
void ImpInterpreter::visit(ForDoStatement* s) {
    ...
    ImpValue n1 = s->e1->accept(this);
    ImpValue n2 = s->e2->accept(this);

    // Verificar que ambas evaluaciones sean de tipo entero
    if (n1.type != TINT || n2.type != TINT) {
        cout << "Error de tipos: operandos en operacion binaria tienen que ser enteros" << endl;
        exit(0);
    }

    int in1 = n1.int_value;
    int in2 = n2.int_value;

    // Solo proceder si in1 <= in2
    if (in1 <= in2) {
        ...
        for (int i = in1; i <= in2; ++i) {
            iter_var.int_value = i;
            env.add_var(s->id, iter_var);
            s->body->accept(this);
        }

        // Eliminar el nivel de entorno de la variable de iteración
        env.remove_level();
    }
...
void ImpInterpreter::visit(FCallstm *s) {
    ...
    ImpVType tt;
    // comparar longitud
    if (fdec->vars.size() != s->arglist.size()) {
        cout << "Error: Numero de parametros incorrecto en llamada a " << fdec->fname << endl;
        exit(0);
    }
    for (it = s->arglist.begin(), varit = fdec->vars.begin(), vartype = fdec->types.begin();
         it != s->arglist.end(); ++it, ++varit, ++vartype) {
    ...
    if (!retcall) {
        cout << "Error: Funcion " << s->id << " no ejecuto RETURN" << endl;
        exit(0);
    }
    retcall = false;
    env.remove_level();
